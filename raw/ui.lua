...
ClickSound.SoundId = "rbxassetid://18998603679"
...
function redzlib:MakeWindow(Configs)
    ...
    local Button = MakeDrag(Create("ImageButton", ScreenGui, {
        ...
        BackgroundColor3 = Theme["Color Hub 2"],
        AutoButtonColor = false,
        Image = MainFrame.Visible and ShowImage or HideImage  -- الصورة الابتدائية
    }))
    ...
    local function UpdateButtonImage()
        if MainFrame.Visible then
            Button.Image = ShowImage
        else
            Button.Image = HideImage
        end
    end
    ...
    Button.Activated:Connect(function()
        ...
        UpdateButtonImage()  -- تحديث الصورة
    end)
    
    -- تحديث الصورة لما الواجهة تتغير
    MainFrame:GetPropertyChangedSignal("Visible"):Connect(UpdateButtonImage)
	...
	function Window:MakeTab(paste, Configs)
		...
		function Tab:AddDropdown(Configs)
				...
				local function UpdateLabel()
					if MultiSelect then
						...
						elseif #list <= 2 then
							...
							ActiveLabel.Text = #list .. " selected"
						end
					else
			...
			function Dropdown:Remove(Option)
				...
				local TargetName = type(Option) == "string" and Option or nil
				for Name, Value in pairs(Opts) do
					if Name == TargetName or Value.index == Option then
						RemoveOption(Name, Value.Value)
						break
					end
				end
			end
		...
		function Tab:AddKeybind(Configs)
			...
			if CheckFlag(Flag) then
				local FlagVal = GetFlag(Flag)
				if FlagVal and Enum.KeyCode[FlagVal] then
					KDefault = Enum.KeyCode[FlagVal]
				end
			end
			...
			local Button, LabelFunc = ButtonFrame(Container, KName, KDesc, UDim2.new(1, -90))
			...
			local KeyFrame = InsertTheme(Create("Frame", Button, {
				...
				AnchorPoint = Vector2.new(1, 0.5),
				BackgroundColor3 = Theme["Color Stroke"]
			}), "Stroke")Make("Corner", KeyFrame, UDim.new(0, 4))
			...
			local function UpdateKeyLabel(Key)
				local name = Key and Key.Name or "None"
				-- trim "KeyCode." prefix if present and shorten long names
			...
			local function StopListening()
				...
				Listening = false
				CreateTween({KeyFrame, "BackgroundColor3", Theme["Color Stroke"], 0.2})
				UpdateKeyLabel(CurrentKey)
			end
			...
			local function StartListening()
				...
				Listening = true
				KeyLabel.Text = "..."
				CreateTween({KeyFrame, "BackgroundColor3", Theme["Color Theme"], 0.2})
			...
			function Keybind:Callback(...) Funcs:InsertCallback(Callback, ...)(CurrentKey) end
			function Keybind:Set(Val1, Val2)
				if type(Val1) == "string" and type(Val2) == "string" then
					LabelFunc:SetTitle(Val1)
					LabelFunc:SetDesc(Val2)
				elseif type(Val1) == "userdata" and pcall(function() return Val1.EnumType end) then
					SetKey(Val1)
				elseif type(Val1) == "function" then
					Callback = Val1
				end
